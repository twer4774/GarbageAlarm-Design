...
//            buttons[i].addTarget(self, action: #selector(SweetAlert.pressed(_:, index index:IndexPath)), for: UIControlEvents.touchUpInside)
    ...
    /*
    @objc open func pressed(_ sender: UIButton!,index index: IndexPath!) {
        self.closeAlert(sender.tag, indexPath: index)
    }
    */
...
//        memo = ud.set(textFieldText, forKey: "memo\(indexPath)")
       ...
       self.textFieldText =  self.textField.text!
        ...
        if userOtherAction != nil{
            ...
            if( isOtherButton == false && isTheOtherButton == false){
                ...
                ud.set(memo, forKey: "memo\(indexPath!)")

            } else if ( isOtherButton == false && isTheOtherButton == true){
                ud.set(nil, forKey: "memo\(indexPath!)")
            }

//            //지우기 클릭시 메모지우기
//            if( isTheOtherButton == true && isOtherButton == false){
//                ud.set(nil, forKey:"memo(\(indexPath!)")
//            }

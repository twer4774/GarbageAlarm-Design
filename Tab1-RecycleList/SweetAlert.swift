...
open class SweetAlert: UIViewController {
    ...
    var userOtherAction:((_ isOtherButton: Bool, _ isTheOtherButton: Bool) -> Void)? = nil
    var userAlarmAction:((_ isOtherButton: Bool, _ isTheOtherButton: Bool) -> Void)? = nil
    let kFont = "Helvetica"
    var textField: UITextField = UITextField()
    var memo = "메모"
    var textFieldText = "텍스트필드"
    var indexAction:((_ indexPath: IndexPath) -> Void)? = nil
    ...
    fileprivate func resizeAndRelayout() {
        ...
        if self.subTitleTextView.text.isEmpty == false {
            ...
            textViewHeight = ceil(rect.size.height) + 11
        ...
        textField.frame = CGRect(x: x, y: y, width: width, height: textViewHeight)
        ...
        if buttons.count == 2 {
            ...
            totalWidth = buttonRect[0].size.width + buttonRect[1].size.width + buttonRect[2].size.width + kWidthMargin + 70.0
        }
        else{
    ...
    func localNotification(){
        ...
        if #available(iOS 10, *){
            ...
            _ = self.datePicker.date.timeIntervalSinceNow
            ...
            let selHour = formatter.string(from: datePicker.date)
            newDate.hour = Int(selHour)
            //분
            formatter.dateFormat = "mm"
            let selMinute = formatter.string(from: datePicker.date)
    ...
    func closeAlert(_ buttonIndex:Int){
     ...
     /*
        if userAction !=  nil {
            ...
            if isOtherButton == true && alarmSwitch.isOn == true{
               localNotification()
            }else if isOtherButton == true && alarmSwitch.isOn == false{
                
                UNUserNotificationCenter.current().removePendingNotificationRequests(withIdentifiers: ["alarm"])
            }
        }
       */
        if userAlarmAction != nil{
            let isOtherButton = buttonIndex == 1 ? true : false
            let isTheOtherButton = buttonIndex == 2 ? true: false
            SweetAlertContext.shouldNotAnimate = true
        ...
        if userOtherAction != nil{
            let ud = UserDefaults.standard
            
            let isOtherButton = buttonIndex == 1 ? true : false
            let isTheOtherButton = buttonIndex == 2 ? true: false
            SweetAlertContext.shouldNotAnimate = true
            userOtherAction!(isOtherButton, isTheOtherButton)
            SweetAlertContext.shouldNotAnimate = false
            
            //저장버튼 클릭시 메모 저장
            if( isOtherButton == false && isTheOtherButton == false){
                memo = self.textField.text!

                ud.set(memo, forKey: "memo\(indexPath.row)")
                
                
            }
            //지우기 클릭시 메모 지우기
            else if ( isOtherButton == false && isTheOtherButton == true){
                ud.set(nil, forKey: "memo\(indexPath.row)")
            }
    ...
    //3개 버튼있는 알람창
    open func showAlert(_ title: String, subTitle: String?, style: AlertStyle,buttonTitle: String,buttonColor: UIColor,otherButtonTitle:
        String?, theOtherButtonTitle: String?, index: IndexPath!, action: ((_ isOtherButton: Bool, _ isTheOtherButton: Bool) -> Void)? = nil) -> SweetAlert
    {
        self.showAlert(title, subTitle: subTitle, style: style, buttonTitle: buttonTitle,buttonColor: buttonColor,otherButtonTitle:
            otherButtonTitle, otherButtonColor: UIColor.red, theOtherButtonTitle:
            theOtherButtonTitle, theOtherButtonColor: UIColor.darkGray, index: index)
        userOtherAction = action
    ...
    open func showAlert(_ title: String, subTitle: String?, style: AlertStyle,buttonTitle: String, buttonColor: UIColor, otherButtonTitle:
        String?, otherButtonColor: UIColor?, theOtherButtonTitle:
        String?, theOtherButtonColor: UIColor?, index: IndexPath!, action: ((_ isOtherButton: Bool, _ isTheOhterButton: Bool) -> Void)? = nil){
//        userAction = action
        userOtherAction = action
        let window: UIWindow = UIApplication.shared.keyWindow!
        window.addSubview(view)
        ...
        if theOtherButtonTitle != nil && theOtherButtonTitle!.isEmpty == false {
            let button: UIButton = UIButton(type: UIButtonType.custom)
            button.setTitle(theOtherButtonTitle, for: UIControlState())
            button.backgroundColor = theOtherButtonColor
            button.addTarget(self, action: #selector(SweetAlert.pressed(_:)), for: UIControlEvents.touchUpInside)
            button.tag = 2
            buttons.append(button)
        }
    ...
    @objc func datePickerChanged(datePicker: UIDatePicker){
        ...
        print("date = \(datePicker.date+9*60*60)")
    }
